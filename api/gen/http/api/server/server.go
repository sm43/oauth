...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			{"Authenticate", "POST", "/oauth/redirect"},
			{"Details", "GET", "/details"},
		...
		Details:      NewDetailsHandler(e.Details, mux, decoder, encoder, errhandler, formatter),
		CORS:         NewCORSHandler(),
	}
}
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountDetailsHandler(mux, h.Details)
	MountCORSHandler(mux, h.CORS)
}

// MountAuthenticateHandler configures the mux to serve the "api" service
// "Authenticate" endpoint.
func MountAuthenticateHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := handleAPIOrigin(h).(http.HandlerFunc)
	if !ok {
...
// and calls the "api" service "Authenticate" endpoint.
...
// MountDetailsHandler configures the mux to serve the "api" service "Details"
// endpoint.
func MountDetailsHandler(mux goahttp.Muxer, h http.Handler) {
	f, ok := handleAPIOrigin(h).(http.HandlerFunc)
	if !ok {
		f = func(w http.ResponseWriter, r *http.Request) {
			h.ServeHTTP(w, r)
	...
	mux.Handle("GET", "/details", f)
}

// NewDetailsHandler creates a HTTP handler which loads the HTTP request and
// calls the "api" service "Details" endpoint.
func NewDetailsHandler(
	...
	var (
		decodeRequest  = DecodeDetailsRequest(mux, decoder)
		encodeResponse = EncodeDetailsResponse(encoder)
		encodeError    = EncodeDetailsError(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "Details")
		ctx = context.WithValue(ctx, goa.ServiceKey, "api")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil {
...
// handleAPIOrigin applies the CORS response headers corresponding to the
// origin for the service api.
func handleAPIOrigin(h http.Handler) http.Handler {
